...
class SimGNNTrainer(object):
    ...
    def transfer_to_torch(self, data, target=True):
        """
    ...
    def predict(self, data):
        """
        Predict on pair of graphs.
        """
        self.model.eval()
        scaling_factor = 0.5*(len(data["labels_1"]) + len(data["labels_2"]))
        data = self.transfer_to_torch(data, target=False)
        prediction_log = self.model(data)
        prediction_log = prediction_log.detach().numpy()
        prediction_norm = -np.log(prediction_log)/np.log(np.exp(1))
        prediction = prediction_norm*scaling_factor
        return prediction
