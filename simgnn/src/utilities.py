...
class data2:
    ...
    def create_batches(self, batch_size=16):
        ...
        batches = []
        for graph in range(0, len(self.training_graphs), batch_size):
            batches.append(self.training_graphs[graph:graph+batch_size])
        return batches
...
def convert_to_keras(data, global_labels):
        ...
        # print("max edge index 1: {}".format(max(max(edges_1))))
        ...
        # print("max edge index 2: {}".format(max(max(edges_2))))
        ...
        # print("len labels 1: {}".format(len(data["labels_1"])))
        for n in data["labels_2"]:
            features_2.append([1.0 if global_labels[n] == i else 0.0 for i in global_labels.values()])
        # print("len labels 2: {}".format(len(data["labels_2"])))
