...
def traverse(dag, successor, modules):
    ...
    if len(preds) == 1:
        ...
        if re.search("^ConvBlock.*", function):
            ...
            f = func_dict['filters']
            k = func_dict['kernel_size']
            modules[successor] = DeconvBlock(modules[preds[0]], f, kernel_size=k, deconv_name=successor)(
                modules[preds[0]])
            print('connecting {} to {}'.format(modules[successor], modules[preds[0]]))
    ...
    with tf.device('/gpu:{}'.format(gpuID)):
        ...
        if compile:
            model.compile(optimizer=Adam(lr=1e-4), loss='binary_crossentropy',
                          metrics=['accuracy', metrics.Precision(), metrics.Recall(), metrics.TruePositives(),
