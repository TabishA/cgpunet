...
def add_layers(A, B, add_mode='conv', add_name='add'):
    ...
    if a_channels == b_channels or a_channels == 1 or b_channels == 1:
        ...
        B = Conv2D(a_channels, kernel_size=(1, 1), padding="same", name=add_name + '_Conv1x1',
                   kernel_initializer='he_normal', use_bias=False)(B)
        B = BatchNormalization(axis=3, name=add_name + '_Conv1x1_bn')(B)
        return add([A, B], name=add_name), 1
    else:
        A = Conv2D(b_channels, kernel_size=(1, 1), padding="same", name=add_name + '_Conv1x1',
                   kernel_initializer='he_normal', use_bias=False)(A)
...
def ConvBlock(filters, kernel_size, use_batchnorm=True, conv_name='conv'):
    def layer(x):
        x = Conv2D(filters, kernel_size, padding="same", name=conv_name + '_Conv2D', kernel_initializer='he_normal',
...
def ResBlock(input_layer, filters, kernel_size, use_batchnorm=True, res_name='res'):
    def layer(x):
        ...
        x = Conv2D(filters, kernel_size, padding="same", name=res_name + '_Conv2D', kernel_initializer='he_normal',
                   use_bias=not (use_batchnorm))(x)
        if use_batchnorm:
...
def DeconvBlock(input_layer, filters, kernel_size, use_batchnorm=True, deconv_name='deconv'):
    def layer(x):
        x = Conv2D(filters, kernel_size, padding="same", name=deconv_name + '_Conv2D', kernel_initializer='he_normal',
                   use_bias=not (use_batchnorm))(input_layer)
        if use_batchnorm:
