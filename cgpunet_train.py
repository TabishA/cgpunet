...
class CNN_train():
    ...
    def __init__(self, dataset_path, img_format, mask_format, verbose=True, input_shape=(256, 256, 1),
                 target_shape=(256, 256, 1), batchsize=16, batchsize_valid=16):
        ...
        self.trainGenerator = DataGenerator(slices_fn=train_imgs, segments_fn=labels_train, batch_size=self.batchsize,
                                            input_shape=self.input_shape, target_shape=self.target_shape)
        self.validGenerator = DataGenerator(slices_fn=valid_imgs, segments_fn=labels_valid,
                                            batch_size=self.batchsize_valid, input_shape=self.input_shape,
