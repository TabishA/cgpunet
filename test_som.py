...
dp = 'D:\\Thesis\\Code\\Data\\Dataset\\data_retina_train'
eval_f = CNNEvaluation(gpu_num=1, dataset_path=dp, img_format='*.tif', mask_format='*.tif', input_shape=(128, 128, 1),
                       target_shape=(128, 128, 1))

pop = [Individual(network_info, False) for _ in range(200)]

test_som = SOM(pop, network_info, N=250)
# tpop = [Individual(network_info, False) for _ in range(40)]
# test_som.fit(tpop,generation=0)
all_populations = get_files('./Data/p_files_netlists/', '*.p')
# all_populations.sort()

for i, p in enumerate(all_populations):
    loaded_pop = pickle.load(open(p, 'rb'))
    test_som.map_population(population=loaded_pop, gen=i)
