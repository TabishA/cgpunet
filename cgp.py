...
class CGP(object):
    ...
    def _evaluation(self, pop, eval_flag, init_flag=False):
        ...
        DAG_list = []
        model_names = []
        active_index = np.where(eval_flag)[0]  # Indices where eval_flag is true, only those candidates to be evaluated
    ...
    def check_memory(self, individual):
        ...
        arguments = (
            individual.active_net_list(), self.eval_func.batchsize, self.eval_func.input_shape,
            self.eval_func.target_shape,
            return_dict)
    ...
    def modified_evolution(self, mutation_rate=0.01, log_file='./log.txt', arch_file='./arch.txt', load_population='',
        ...
        with open('child.txt', 'w') as fw_c:
            ...
            if not load_population:
                ...
                # TODO: Why use a nested loop here?
        ...
        for candidate_number in range(candidate_index, len(self.pop), self.lam):
            while current_temp > final_temp:
                ...
                for j in range(min(self.lam, len(self.pop) - candidate_number)):
                    ...
                    # If mutation is neutral only, assign the same fitness
                    if mutated[j] and not eval_flag[j]:
                        mutant.eval = self.pop[candidate_number + j].eval
                ...
                # display and save log
                print(self._log_state(log_dir, index=candidate_number, current_temp=current_temp,
