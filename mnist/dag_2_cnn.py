...
def traverse(dag, successor, modules):
    ...
    if len(preds) == 1:
        ...
        if re.search("^ConvBlock.*", function):
            ...
            print('connecting {} to {} and {}'.format(modules[successor], modules[preds[0]], modules[preds[0]]))
        elif re.search("^FC.*", function):
            units = func_dict['units']
            if re.search("^FCB.*", function):
                modules[successor] = DenseBlock(modules[preds[0]], units, name=successor)
            else:
                modules[successor] = DenseBlock(modules[preds[0]], units, name=successor, use_batchnorm=False)
            print('connecting {} to {}'.format(modules[successor], modules[preds[0]]))
    elif len(preds) == 2:
        if not preds[0] in m_keys: 
            modules = traverse(dag, preds[0], modules)
...
def dag_2_cnn(dag, gpuID, input_shape=(256,256,1), pretrained_weights = None, compile=True):
    ...
    with tf.device('/gpu:{}'.format(gpuID)):
        ...
        if compile:
            model.compile(optimizer=Adam(lr=1e-4), loss='categorical_crossentropy', metrics=['accuracy', metrics.Precision(), metrics.Recall(), metrics.TruePositives(), metrics.TrueNegatives(), metrics.FalsePositives(), metrics.FalseNegatives(), metrics.AUC()])
