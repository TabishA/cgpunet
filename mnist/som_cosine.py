...
class Node():
    ...
    def adapt_weights(self, input_weights, winner_node, t, t_max):
        ...
        vec_indices = np.where(self.weights.any(axis=0))[0]
        
        assert np.max(vec_indices) == len(vec_indices) - 1

        self.vectors = []
        for i in range(len(vec_indices)):
            self.vectors.append(list(self.weights[:,i]))
...
class SOM(object):
    ...
    def init_weights(self):
        ...
        for i, ind in enumerate(self.init_population):
            weights = np.zeros((4, self.network_info.max_active_num))
            ...
            self.node_list.append(Node((coord_x,coord_y), weights, vecs))
    ...
    def fit(self, population, t_max):
        ...
        for t in range(t_max):
            ...
            for ind in population:
                input_matrix = np.zeros((4, self.network_info.max_active_num))
                active_net_list = ind.active_net_list()
                ...
                for node in self.node_list:
                    node.adapt_weights(input_matrix, bmu, t, t_max)
