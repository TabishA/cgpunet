...
def get_model_memory_usage(net_list, fc_layers, batch_size, input_shape, return_dict):
    try:
        model = dag_2_cnn(cgp_2_dag(net_list, fc_layers), 0, input_shape, compile=False)
...
class Individual(object):
    ...
    def init_gene(self):
        ...
        max_fc_layers = min(self.net_info.max_active_num - len(self.active_net_list()), self.max_fc_layers)
        if max_fc_layers > 0:
            num_fc_layers = np.random.randint(0,max_fc_layers)
            self.fc_layers = list(np.random.choice(self.net_info.fc_func_type, num_fc_layers))
    ...
    def check_pool(self):
        G = cgp_2_dag(self.active_net_list(), self.fc_layers)
        max_pool_num = 0
        for n in G.nodes():
    ...
    def mutation(self, mutation_rate=0.01):
        ...
        for n in range(self.net_info.node_num + self.net_info.out_num):
            ...
            #mutation for fully connected layers
            if np.random.rand() < mutation_rate:
                mutation_type = np.random.randint(0,3)

                #swap one fc layer with a random fc layer
                if mutation_type == 0 and len(self.fc_layers) != 0:
                    gene_index = np.random.randint(0,len(self.fc_layers))
                    self.fc_layers[gene_index] = np.random.choice(self.net_info.fc_func_type)
                elif mutation_type == 1 and len(self.fc_layers) != 0: #remove random fc layer
                    gene_index = np.random.randint(0,len(self.fc_layers))
                    self.fc_layers.remove(self.fc_layers[gene_index])
                else: #add random fc layer
                    self.fc_layers.append(np.random.choice(self.net_info.fc_func_type))
...
class CGP(object):
    ...
    def _evaluation(self, pop, eval_flag, init_flag=False):
        ...
        for i in active_index:
            G = cgp_2_dag(pop[i].active_net_list(), pop[i].fc_layers)
            DAG_list.append(G)
    ...
    def evaluate_novelty(self, next_generation = None):
        ...
        for p in self.pop:
            DAGs[p] = cgp_2_dag(p.active_net_list(), p.fc_layers)
        ...
        if next_generation is None:
            ...
            if k_a > 0:
                for a in self.search_archive:
                    DAGs_archive.append(cgp_2_dag(a.active_net_list(), a.fc_layers))
            for individual in next_generation:
                G = cgp_2_dag(individual.active_net_list(), individual.fc_layers)
    ...
    def check_memory(self, individual):
        ...
        arguments = (individual.active_net_list(), individual.fc_layers, self.eval_func.batchsize, self.eval_func.input_shape, return_dict)
