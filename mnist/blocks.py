...
def DenseBlock(input_layer, units, name, use_batchnorm=True):
    flattened = False
    if len(input_layer.shape) > 2:
        x = Flatten()(input_layer)
        flattened = True
        
    #this hardcoded for num_classes=10 would need to be changed for other datasets
    if flattened:
        x = Dense(units, name=name)(x)
        if use_batchnorm: x = BatchNormalization(name=name + '_bn')(x)
    else:
        x = Dense(units, name=name)(input_layer)
        if use_batchnorm: x = BatchNormalization(name=name + '_bn')(x)
    
    if units == 10:
        x = Activation('softmax', name=name + '_softmax')(x)
    else:
